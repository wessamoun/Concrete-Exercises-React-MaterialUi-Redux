...
function SameTargetExercises(props) {
  ...
  useEffect(() => {
    props.exercise.length > 0 &&
      setItems(data.filter((e) => e.target === props.exercise[0].target).slice(0,10));
  }, [props.exercise]);
  return (
    <>
      {items.length > 0 && (
        <>
          <Typography variant="h4" textTransform={"capitalize"} mt={"50px"}>
            Some Similar Target Muscle exercises
          </Typography>
...
export default SameTargetExercises;
